...
public class PsychoCut : MonoBehaviour, Move
{
    ...
    void Start()
    {
        ...
        addMoveStat("Damage");
        moveStats[0].Calc = (x) => { return Mathf.CeilToInt((float)(math.pow(1.05, CalcMulti(player.leafStoneAmount)))* .5f * (pokemon.@base.Attack + player.might + 200f) * 2 / (50) * (x + 1)); };
        addMoveStat("Cooldown");
        moveStats[1].Calc = (x) => { return (float)(math.pow(.95, CalcMulti(0))) * (.5f * 100f / pokemon.@base.Speed) * ((float)(Unity.Mathematics.math.pow(.9, x))); };
        addMoveStat("Pierce");
    ...
    public void Attack()
    {
        ...
        PsychoCutInstance ri = b.GetComponent<PsychoCutInstance>();
        ri.psychocut = this;
        ri.pierce = pierce;
        ri.goAwayTime = Time.time + 4; ;

        float flip = 1;
        if (player.closestMob.dir.y < 0)
        {
            flip = -1;
        }
        Debug.Log(Vector2.Angle(player.closestMob.dir * -1, Vector2.right));
        b.transform.eulerAngles = new Vector3(0, 0, Vector2.Angle(player.closestMob.dir, Vector2.right) * flip + 180);
